    ...
    async fn test_cecho_handling() {
        ...
        let client_handle = tokio::spawn(async move {
            ...
            let command = InMemDicomObject::command_from_element_iter([
                ...
                DataElement::new(tags::COMMAND_DATA_SET_TYPE, VR::US, dicom_value!(U16, [0x0101])), // No dataset
            ]);
            ...
            command.write_dataset_with_ts(&mut command_data, ts).expect("Failed to write command");
            ...
            association.send(&Pdu::PData { data: vec![msg] }).await.expect("Failed to send C-ECHO");
            
            // Receive response
            let pdu = association.receive().await.expect("Failed to receive response");
            match pdu {
                Pdu::PData { data } => {
        ...
        let server_handle = tokio::task::spawn_blocking(move || {
            ...
            rt.block_on(async {
                server_clone.run_store_sync(std_stream, &out_dir_clone).await.unwrap();
            });
        });
